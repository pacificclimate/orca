...
@click.option("-v", "--variable", help="Varible + time range string (ex. tasmax[0:55114])")
@click.option("-t", "--lat", help="Latitude slab as index (ex. [90:100])")
@click.option("-n", "--lon", help="Longitude slab as index (ex. [90:100])")
def process(connection_string, unique_id, variable, lat, lon):
    """CLI for orca"""
    process_request(connection_string, unique_id, variable, lat, lon)
